...
def create_dataset(filelist, labels):
    batch_x = np.asarray([np.load(f) for f in filelist])      
...
if __name__ == "__main__":
    ...
    parser.add_argument('--list', action='store', type=str, default='lists_selected_bandwidth/files_lists_tagmap=binary_classification.npy', dest='input_file_list', help='load the file list corresponding to the experiments, default=lists_selected_bandwidth/files_lists_tagmap=binary_classification.npy')

    args = parser.parse_args()

    [_, _, x_test_filelist, y_train, _, y_test] = np.load (args.input_file_list, allow_pickle = True)
    ...
    test_dataset = create_dataset(x_test_filelist, y_test)
